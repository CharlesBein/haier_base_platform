...
public class LogUtil {
    private static final Logger logger = LoggerFactory.getLogger("JSH-SERVICE");
...
//            curLogger = loggerAction;
//        }
//        else if(retMap.containsKey("service")){
//            curLogger = loggerService;
//        }
        ...
        for (StackTraceElement ste : stack) {
            if ((ste.getClassName().indexOf("LogUtil")) == -1 && ste.getClassName().indexOf("Thread") == -1) {
                ...
                */
                ...
                //建立返回串
                StringBuffer buf = new StringBuffer();
//                if(retMap.containsKey("action")){//加入用户名等
//
//                    User u = ActionUtil.getLoginUser();
//                    if(u !=null){
//                        buf.append("[").append(u.getUserName()).append("]").append("[").append(u.getUserType()).append("]");
//                    }
//                }
                //模块

                String logTime = DateUtil.dateToString(new Date(System.currentTimeMillis()));
    ...
    private static String getCauseMsg(Throwable e) {
        ...
        String stacks = Arrays.asList(e.getStackTrace()).stream().filter(el -> el.getClassName().contains(PACKAGE_FLAG)).filter(el -> !el.getClassName().contains("framework")).filter(el -> el.getLineNumber() != -1).map(el -> {
            StringBuffer buf = new StringBuffer();
